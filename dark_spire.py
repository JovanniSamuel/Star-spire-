...
try:
    ...
    pygame.mixer.music.play(-1)  # ensures that it loops forever
except pygame.error as e:
    print(f"Warning: could not load main menu music: {e}")


def load_image(filename, size):
    # BUG FIX: this used to have no error handling at all, so any missing/renamed
    # asset file would crash the entire game on startup. Now it falls back to a
    # visible placeholder square so the game keeps running.
    try:
        surf = pygame.image.load(filename).convert_alpha()
    except (pygame.error, FileNotFoundError) as e:
        print(f"Warning: could not load image '{filename}' ({e}); using placeholder.")
...
# ---------------------------------------------------------------------------
SUMMON_TYPE = {
    "rabbit":    {"hp": 220, "attack": 6,  "defense": 12, "speed": 4,  "magicpower": 5},
    "lion":      {"hp": 100, "attack": 30, "defense": 10, "speed": 10, "magicpower": 12},
    "crow":      {"hp": 80,  "attack": 16, "defense": 10, "speed": 20, "magicpower": 10},
    "butterfly": {"hp": 10,  "attack": 6,  "defense": 2,  "speed": 8,  "magicpower": 25},
}

ULT_SUMMON_TYPE = {
    "Titan": {"hp": 320, "attack": 50, "defense": 20, "speed": 40, "magicpower": 50}
}
...
# Classes
# ---------------------------------------------------------------------------
class Characters(pygame.sprite.Sprite):  # starts the character class
    def __init__(self, name, load_sprite, size, x, y, hp=100, attack=10, magicpower=10,
                 ...
                 ult_cooldown=60000, ult_ready_timer=0):
        super().__init__()
        self.name = name
...
class Mobs(Characters):
    def __init__(self, name, load_sprite, size, x, y, hp=100, attack=10, magicpower=10,
                 speed=10, mana=50, defense=5, mana_regen=5,
        ...
        super().__init__(name=name, load_sprite=load_sprite, size=size, x=x, y=y, hp=hp,
                          ...
                          poison_ticks_left=poison_ticks, poison_tick_timer=poison_tick_timer,
    ...
    def enemy_attack(self, target):
        if getattr(target, "invisibility", False):
            if random.random() < 1 / 10:  # 10% chance to hit an invisible target
                ...
                if self.can_poison:
                    target.poison = True
                    target.poison_damage = max(int(target.max_hp) // 32, 1)  # 1/32 max hp as poison damage
    ...
    def update_ai(self, player, current_time):
        # invisible players: only finish an attack already in progress, then lose track
...
class Summoner(Characters):
    ...
    def summon_familiar(self, summon_type, sprite_paths, size, x, y, mana_cost=30):
        ...
        if len(self.summons) >= self.MAX_SUMMONS:
            print(f"{self.name} already has the max of {self.MAX_SUMMONS} summons out!")
            return None
        ...
        print(f"{self.name} summoned a {summon_type}! ({len(self.summons)}/{self.MAX_SUMMONS})")
        return new_summon
...
class Summons(pygame.sprite.Sprite):
    def __init__(self, summon_type, sprite_path, size, x, y, stat_table=None):
        ...
        self.image = load_image(sprite_path, size)
        self.rect = self.image.get_rect()
        self.rect.x = x
        self.rect.y = y
        self.hp = stats["hp"]
...
class Start_button(pygame.sprite.Sprite):  # Start button class
    def __init__(self, game_state_manager):
        ...
        try:
            self.image = load_image("Start_button2.png", (300, 100))
...
class Character_button(pygame.sprite.Sprite):  # Character button class
    def __init__(self, game_state_manager):
        ...
        self.game_state_manager = game_state_manager
        try:
            self.image = load_image("Select_character_button2.png", (150, 100))
...
class Floor1(pygame.sprite.Sprite):
    def __init__(self):
        ...
        try:
            ...
            print("Dungeon loaded successfully -- Forest")
        except pygame.error as e:
            print(f"Error loading dungeon 1 -- forest: {e}")
        self.rect = self.image.get_rect()
        self.rect.x = 0
        self.rect.y = 0
...
class Menu_Background(pygame.sprite.Sprite):  # Menu background class
    def __init__(self):
        super().__init__()
        try:
            self.image = load_image("darkspireposter.png", (WIDTH, HEIGHT))
...
start_button = Start_button(game_state_manager)
character_button = Character_button(game_state_manager)
...
# for your real art; missing files now just fall back to a placeholder square
# instead of crashing (see load_image).
...
sprite_paths = {"rabbit": "rabbit.png", "lion": "lion.png", "crow": "crow.png", "butterfly": "butterfly.png"}

mobs_group = pygame.sprite.Group()
mobs_group.add(Mobs("Goblin", "goblin.png", (64, 64), 700, 300))  # demo target -- add more/replace as needed
...
while running:
    ...
    state = game_state_manager.get_state()   # moved above the event loop -- was previously read
                                              # inside the event loop before it had ever been set
        ...
        if event.type == pygame.KEYDOWN:
            if state == "character_select":
                ...
                if isinstance(active_player, Assain):
                    ...
                    # h/j/k/l: summon rabbit/lion/crow/butterfly familiars
                    if event.key == pygame.K_1 and skill_target is not None:
                        ...
                        active_player.summon_familiar("rabbit", sprite_paths, (48, 48),
                                                       active_player.rect.x - 40, active_player.rect.y)
                    elif event.key == pygame.K_j:
                        active_player.summon_familiar("lion", sprite_paths, (48, 48),
                                                       active_player.rect.x - 40, active_player.rect.y)
                    elif event.key == pygame.K_k:
                        active_player.summon_familiar("crow", sprite_paths, (48, 48),
                                                       active_player.rect.x - 40, active_player.rect.y)
                    elif event.key == pygame.K_l:
                        active_player.summon_familiar("butterfly", sprite_paths, (48, 48),
                                                       active_player.rect.x - 40, active_player.rect.y)
    ...
    if state == "menu":
        ...
        floor1_group.update()
        floor1_group.draw(screen)   # fixed: floor1 is a single Sprite, not a Group -- it has no .draw()
        ...
        if isinstance(active_player, Assain):
            ...
            active_player.update_spell_shield(current_time)  # BUG FIX: this used to live in
            # an unreachable "shielded" state that game_state_manager could never enter
